...
- `R_1 = 10 kΩ`, `R_2 = 10 kΩ`, jeweils 1 %
- Lastwiderstände 100 kΩ, 22 kΩ und 10 kΩ, jeweils 1 %
- Modellquelle: `R_i = 1,0 kΩ` in Reihe mit einer 5-V-Versorgung
...
Teil A verwendet den Spannungsteiler aus Lektion 03.3. `R_1` liegt von 5 V nach `U_out`, `R_2` von `U_out` nach GND. Die Last wird jeweils parallel zu `R_2` eingesetzt.
...
4. Sage qualitativ voraus, in welche Richtung `U_out` mit kleiner werdender Last geht.
...
Berücksichtige bei der genauesten Rechnung den DMM-Eingangswiderstand als zusätzliche Parallellast. Bestimme ausserdem den Ausgangswiderstand `R_out = R_1 || R_2`.

Für Teil B berechnest du aus `U_0 = 5,000 V`, `R_i = 1,0 kΩ` und jeder Last den Strom sowie die erwartete Klemmenspannung.
...
2. Miss `U_out` zunächst ohne zusätzlichen Lastwiderstand.
...
2. Schliesse 10 kΩ an, miss Klemmenspannung und berechne den Laststrom aus `U_K/R_L`.
3. Wiederhole mit 4,7 kΩ.
4. Bestimme `R_i` jeweils aus `(U_0−U_K)/I` und zusätzlich aus der Steigung zwischen den beiden Lastpunkten.
...
| Last | berechnetes `U_out` | gemessenes `U_out` | Abweichung | Bedingung |
|---|---:|---:|---:|---|
...
- Zeichne `U_out` aus Teil A über dem Lastwiderstand auf einer sinnvollen Achse.
